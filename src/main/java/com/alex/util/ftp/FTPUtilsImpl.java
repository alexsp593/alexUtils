...
import com.alex.util.CheckUtils;
import com.alex.util.FTPUtils;
 ...
 * @Deception:FTP工具类的实现
 * @Author:Alex
 * @Date:2018年06月06日21点39分
 */
public class FTPUtilsImpl implements FTPUtils {
    private Logger logger = Logger.getLogger(FTPUtilsImpl.class);
    private FTPClient client;
    private FTPConfig     ftpConfig;


    public FTPUtilsImpl(FTPConfig ftpConfig) throws IOException {
        this.ftpConfig = ftpConfig;
        client = createFTPClien(ftpConfig);
    }

    //创建变连接FTP
    private FTPClient createFTPClien(FTPConfig ftpConfig) {
        ...
        try {
            ...
            if (!FTPReply.isPositiveCompletion(reply)) {
                ...
                client.cwd(ftpConfig.getRemoteDir());
            }
            return client;
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public boolean isExists(String fileName) {
        List<String> list = listFile(ftpConfig.getRemoteDir());
