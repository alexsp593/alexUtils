...
/**
 * @Deception:Java Class与反射相关的一些工具类
 * @Author:Alex
 * @Date:2018年06月14日11点15分
 */
public final class ClassUtils {
    ...
    public final static List<String> getClassNameByFile(String filePath, boolean childPackage) {
        ...
        List<File> files = FileUtils.listFile(filePath, childPackage);
        for (File file : files) {
        ...
        try (JarFile jarFile = new JarFile(jarPath)) {
            ...
            while (entrys.hasMoreElements()) {
                ...
                if (!entryName.endsWith(".class") && !entryName.endsWith("/")) {
                    resource.add(FilePathUtils.commandPath(jarPath) + "!" + entryName);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public final static List<String> getResourceNameByJar(String jarPath, String suffix) {
        ...
        try (JarFile jarFile = new JarFile(jarPath)) {
            ...
            logger.error(ExceptionUtils.stackTraceToString(e, "com.opslab.util"));
        }
        return resource;
    }
